...
import json
import os
import threading  # ★ 추가: TTS 재생을 별도 스레드로 처리하기 위해
...
class MQTT_TTS_Handler:
    ...
    def __init__(self, broker_address, broker_port=1883,
        ...
        # 토픽별 콜백 분기
        self.client.message_callback_add(self.text_topic, self.on_text_message)
    ...
    # 모든 토픽에 대한 기본 메시지 콜백 (디버그용)
    def on_message_catchall(self, client, userdata, msg):
        #print(f"[CatchAll] Topic: {msg.topic}, Payload: {msg.payload}")
        pass

    def on_text_message(self, client, userdata, msg):
        """
        text_topic에 대한 콜백.
        JSON 안의 "data" 필드를 추출해서, 별도 스레드에서 TTS 재생을 진행
        """
        try:
            ...
            # 별도 스레드에서 TTS 재생 (블로킹)
            t = threading.Thread(target=self.play_tts, args=(text,))
            t.start()
    ...
    def play_tts(self, text):
        """
        별도 스레드에서 TTS를 재생하여, 메인 스레드를 블로킹하지 않도록 함.
        """
    ...
    def on_force_stop_message(self, client, userdata, msg):
        ...
        try:
            ...
            print(f"[ForceStop Message] Received: {data}")
    ...
    def start(self):
        ...
        # ★ 메인 스레드를 블로킹하지 않고, 백그라운드 스레드에서 MQTT 루프 시작
        self.client.loop_start()

        print("MQTT loop started in the background thread.")
