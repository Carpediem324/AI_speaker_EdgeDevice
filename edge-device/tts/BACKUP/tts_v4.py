...
MP3_VOLUNE = os.getenv("MP3_VOLUNE", 1.0)  # 대기 사운드 볼륨
...
def tts_fast(text):
    ...
    원본 16kHz, 16비트, Mono PCM 데이터를 그대로 재생.
    ...
    if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
        ...
        print("✅ [TTS 재생 시작]")
        try:
            # 원본 데이터를 그대로 재생하므로 samplerate를 16000으로 설정합니다.
            sd.play(audio_np, samplerate=16000, device=None)
            sd.wait()  # 재생이 끝날 때까지 대기
            sd.stop()  # 재생 후 스피커 초기화
            print("✅ [TTS 재생 완료]\n")
...
class MQTT_TTS_Handler:
    ...
    def on_text_message(self, client, userdata, msg):
        ...
        텍스트 토픽 수신 시:
         ...
         - TTS 시작 시각 기록 및 TTS 재생 완료 후 피니쉬 토픽 발행
        """
        try:
