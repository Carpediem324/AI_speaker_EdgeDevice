...
RMS_THRESHOLD = 500        # RMS 임계치 (이 수치 이하면 침묵으로 판단)
...
class MQTTClientHandler:
    def __init__(self, debug=False):
        ...
        self.client.subscribe(TOPIC_TTS_FINISHED)  # TTS 피니쉬 토픽 구독
...
class AudioStreamer:
    ...
    def __init__(self, mqtt_handler, led_strip, debug=False,
        ...
        # TTS 피니쉬 토픽 콜백을 초기화 단계에서 한 번만 등록합니다.
        self.mqtt_handler.client.message_callback_add(TOPIC_TTS_FINISHED, self.on_tts_finished_message)

    def debug_print(self, msg):
        if self.debug:
            print(msg)

    def on_tts_finished_message(self, client, userdata, msg):
        try:
            data = json.loads(msg.payload.decode('utf-8'))
            #self.debug_print("수신한 TTS 피니쉬 메시지: " + str(data))
            if data.get("status") == "TTS_FINISHED":
                led_controller.all_off(self.led_strip)
        except Exception as e:
            self.debug_print("TTS 피니쉬 메시지 처리 에러: " + str(e))
