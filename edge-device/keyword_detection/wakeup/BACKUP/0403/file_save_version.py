...
import numpy as np
import soundfile as sf  # 파일 저장(테스트용, 필요 시)
import wave
import io
...
def tts_fast(text):
    ...
    Gain 적용, 이후 WAV 파일로 저장하여 pygame으로 재생.
    """
    ...
    if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
        ...
        # pygame.mixer.Sound 로드 & 재생
        sound_obj = pygame.mixer.Sound(wav_filename)
        print("✅ [TTS 재생 시작]")
        channel = sound_obj.play()
        
        # 재생 끝날 때까지 대기 (동기 방식)
        while channel.get_busy():
            pygame.time.wait(50)  # 50ms 쉬면서 체크
