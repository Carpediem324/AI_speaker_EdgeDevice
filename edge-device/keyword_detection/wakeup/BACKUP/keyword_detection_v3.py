...
# 상수 및 환경변수
# ----------------------
SILENCE_THRESHOLD = 2      # 2초 동안 '침묵'이면 종료
...
class MQTTClientHandler:
    def __init__(self, debug=False):
        ...
        self.client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2, "keyword_mqtt_client_gwangju_250")
...
class AudioStreamer:
    ...
    * 2초 동안 유지 시 종료
    ...
    def __init__(self, mqtt_handler, debug=False,
        ...
        self.silence_duration_required = SILENCE_THRESHOLD  # 2초
            ...
            if diff_rms < DIFF_THRESHOLD or talk_rms < RMS_THRESHOLD:
                ...
                if silence_start is None:
                    ...
                    if time.time() - silence_start > self.silence_duration_required:
                        self.debug_print("2초간 차이 <200 → 침묵 -> 종료")
                        break
            else:
                # 차이가 충분 -> 침묵 아님
                silence_start = None
