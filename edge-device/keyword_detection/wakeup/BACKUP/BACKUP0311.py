...
def main():
    ...
    test_hw = HotwordDetector(
        ...
        model=base_model,
        reference_file="/home/ubuntu/S12P21S001/edge-device/keyword_detection/refer/javix_ref.json",
        threshold=0.7,
        relaxation_time=2
    )
    ...
    while True:
        ...
        if "rms" in result:
            print("RMS in result:", result["rms"])
        else:
            print("RMS not found in result")
        if result["match"]:
            print("Hotword 감지됨! Confidence:", result["confidence"])
            # 마이크 해제 없이 바로 MQTT 스트리밍 시작
            stream_audio_mqtt()
