...
MP3_VOLUNE = 0.9
TOPIC_TTS_FINISHED = os.getenv("TOPIC_TTS_FINISHED")  # 피니쉬 토픽 추가
TTS_VOICE = os.getenv("TTS_VOICE", "ko-KR-SunHiNeural")  # TTS 음성 설정
...
class WaitSound:
    ...
    def play(self):
        self.sound.play(loops=2)  # loops=-1로 하면 무한 반복
...
def tts_fast(text):
    ...
    speech_config.speech_synthesis_voice_name = TTS_VOICE  # TTS_VOICE 환경변수 사용
    ...
    # 자동 스피커 재생 방지를 위해 audio_config에 None 지정
    audio_config = None
    ...
    if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
        ...
        # AudioDataStream의 read_all() 메서드를 사용하여 전체 오디오 데이터를 읽어옵니다.
        ...
        # float64 변환 후 16kHz → 48kHz 업샘플링 (resample_poly 사용)
        ...
        # TTS 재생이 완료된 시점의 tts_start_time 반환 (이를 이용해 완료 메시지 발행 가능)
...
class MQTT_TTS_Handler:
    ...
    def __init__(self, broker_address, broker_port=1883, username=None, password=None, topic=TOPIC_TTS):
        ...
        self.client.on_message = self.on_text_message  # 기본 메시지 콜백: 텍스트 토픽 처리
    ...
    def on_text_message(self, client, userdata, msg):
        ...
        텍스트 토픽 수신 시:
         - 대기 사운드 정지
         - 텍스트 수신 시각 기록
         - TTS 실행 후 TTS 시작 시각 기록 및 TTS 재생 완료 후 피니쉬 토픽 발행
        """
        try:
            ...
            if start_time is not None:
                ...
                if TOPIC_TTS_FINISHED:
                    finish_payload = json.dumps({"status": "TTS_FINISHED", "tts_start_time": start_time})
